  ...
  <div class="wrap-container">
    ...
    if( !empty( $contacts ) ){
      foreach( $contacts as $key => $list ){
        ...
        ?>
            <tr>  
              <td scope="col" class="manage-column column-name" data-href="<?php echo admin_url(); ?>admin.php?page=contact-manager&view=contact-details&list=<?php echo $_GET['list']; ?>&id=<?php echo esc_attr( $user_id ); ?>&email=<?php echo $email; ?>" ><span class="connect-list-gravtar-img"><?php echo get_avatar($email,'96','https://support.brainstormforce.com/wp-content/uploads/2015/07/default-gravtar.png' ); ?></span><?php echo esc_attr( $name ); ?></td>
              <td scope="col" class="manage-column column-email" data-href="<?php echo admin_url(); ?>admin.php?page=contact-manager&view=contact-details&list=<?php echo $_GET['list']; ?>&id=<?php echo esc_attr( $user_id ); ?>&email=<?php echo $email; ?>"><?php echo esc_attr( $email ); ?></td>
              <td scope="col" class="manage-column column-date" data-href="<?php echo admin_url(); ?>admin.php?page=contact-manager&view=contact-details&list=<?php echo $_GET['list']; ?>&id=<?php echo esc_attr( $user_id ); ?>&email=<?php echo $email; ?>"><?php echo esc_attr( $date ); ?></td>
              <input type="hidden" id="delete-contact-nonce" value="<?php echo $delete_contact_nonce; ?>" />
              <td scope="col" class="manage-column column-delete"><a class="action-list delete-contact" style="margin-left: 6px;" data-list-id="<?php echo $key; ?>" data-list="<?php echo $_GET['list']; ?>" data-user-id="<?php echo esc_attr( $user_id ); ?>" data-email="<?php echo $email; ?>" href="#" data-mailer = "<?php echo $mailer;?>"><i class="connects-icon-trash"></i><span class="action-tooltip"><?php _e( "Delete", "smile" ); ?></span></a></td>
            </tr>
            <?php
      }
    } else {
